...
class RetrievalEvaluator:
    def __init__(self):
        ...
        self.llm_path = (
            f"{judge_provider}/{judge_model}" if "/" not in judge_model else judge_model
        )
        self.emb_path = (
            f"{embed_provider}/{embed_model}" if "/" not in embed_model else embed_model
        )
    ...
    def calculate_hit_rate(
        ...
        return (
            1.0
            if any(doc_id in retrieved_ids[:top_k] for doc_id in expected_ids)
            else 0.0
        )
    ...
    async def score(self, case: Dict[str, Any], resp: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None,
                lambda: evaluate(
                    dataset, metrics=[m1, m2], llm=ragas_llm, embeddings=ragas_emb
                ),
            )
